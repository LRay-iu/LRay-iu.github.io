...
$(document).ready(function ($) {
  ...
  // 找到没有指定类名的图片，并添加 Fancybox 功能
  $("img:not(.nofancybox)").each(function () {
    // 获取图片的alt属性
    var alt = this.alt;
    var imgSrc = this.dataset.original || $(this).attr("data-src") || this.src;

    // 将img标签替换为a标签，并添加Fancybox属性和样式
    if (imgSrc) {
      var fancyboxLink = $("<a />", {
        class: "fancybox",
        ...
        target: "_blank",
        href: imgSrc,
        rel: "gallery_clnofasqx00024rfy918v204c noopener",
        title: alt,
      });

      $(this).attr("src", imgSrc);
      fancyboxLink.append($(this).clone().removeAttr("data-original"));
